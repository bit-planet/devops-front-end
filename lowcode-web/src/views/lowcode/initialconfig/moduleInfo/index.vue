<template>
  <div class="system-user-container">
        ...
        <template #default>
          <el-table :data="state.tableData.records" style="width: 100%"
            ...
            <el-table-column type="selection" width="55"/>
             ...
             <el-table-column prop="description" label="模块描述" show-overflow-tooltip></el-table-column>
             <el-table-column prop="backGitDir" label="后端Git目录" show-overflow-tooltip></el-table-column>
             <el-table-column prop="backModuleDir" label="后端模块目录" show-overflow-tooltip></el-table-column>
             <el-table-column prop="frontGitDir" label="前端Git目录" show-overflow-tooltip></el-table-column>
             <el-table-column prop="frontModuleDir" label="前端模块目录" show-overflow-tooltip></el-table-column>
    ...
    <AddDialog ref="addRef" title="测试父组件向子组件传值标题"/>
    <EditDialog ref="editRef" title="错错错"/>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const initTableData = () => {
    ...
    request({
      url: '/lowcode/initialconfig/moduleInfo/page',
      method: 'get',
      params: pageQuery,
    }).then(res => {
  ...
  const onRowDel = (row: Object) => {
    ElMessageBox.confirm(`此操作将永久ID为：“${row.id}”，是否继续?`, '提示', {
    ...
    .then(() => {
      request({
        url: '/lowcode/initialconfig/moduleInfo/'+row.id,
        method: 'delete',
      }).then(res => {
